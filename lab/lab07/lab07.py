...
def link_to_list(link):
    ...
    if link is not Link.empty:
        if isinstance(link.first, Link):
            list_rep.append(link_to_list(link.first))
        else: list_rep.append(link.first)
        if link.rest is not Link.empty:
            list_rep += (link_to_list(link.rest))
    return list_rep
...
def store_digits(n):
    ...
    j=10
    while n // j > 10: j*=10
    return Link(n // j , store_digits(n%j))
...
def is_bst(t):
    ...
    False
    """
    if not t.branches or len(t.branches) == 1 and is_bst(t.branches[0]):
        return True
    elif len(t.branches) > 2:
        return False
    elif t.branches[0].label <= t.label and t.branches[1].label > t.label:
        if False not in [is_bst(subtree) for subtree in t.branches]:
            return True
