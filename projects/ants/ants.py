    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt):
                if QueenAnt.true_queen is insect: return
...
class Bee(Insect):
    ...
    def action(self, colony, directed_forward=True):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while place is not hive:
            if self.min_range <= curr_range <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        ant_place = self.place #need a pointer to Ant's place since death removes self.place
        Ant.reduce_armor(self, amount)
        if self.armor == 0:
            for b in list(ant_place.bees):
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing except as a high armor defense block."""
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if QueenAnt.true_queen is self:
            ...
            while place.exit:
                place = place.exit
                if place.ant:
                    ants_to_double.append(place.ant)
                    if place.ant.is_container and place.ant.contained_ant:
                        ants_to_double.append(place.ant.contained_ant)

            for ant in [a for a in ants_to_double if a not in QueenAnt.doubled_ants]:
                ant.damage *= 2
    ...
    def reduce_armor(self, amount):
        ...
        if QueenAnt.true_queen is self and self.armor == 0:
           bees_win()
        # END Problem 13
...
def make_scare(action, bee):
    ...
    def scared_action(colony):
        return action(colony, directed_forward=False)
...
def apply_effect(effect, bee, duration):
    ...
    def timed_action(colony):
        nonlocal duration
        if duration > 0:
            action = effect(original_action, bee)
            action(colony)
        else:
            original_action(colony)
        duration -= 1
    bee.action = timed_action
    # END Problem EC
